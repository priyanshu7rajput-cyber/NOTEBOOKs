...
router.get('/status', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        ...
        const { data: connection, error } = await supabase_service_1.supabaseAdmin
...
router.delete('/status', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
        ...
        await supabase_service_1.supabaseAdmin
            .from('google_connections')
            .delete()
            .eq('user_id', userId);
...
router.get('/auth', async (req, res) => {
    try {
        ...
        const oauth2Client = (0, google_service_1.getOAuth2Client)();
        const authUrl = oauth2Client.generateAuthUrl({
            access_type: 'offline',
            prompt: 'consent',
            scope: google_service_1.GOOGLE_SCOPES,
            state: userId,
        });
...
router.get('/callback', async (req, res) => {
    ...
    try {
        ...
        const { error: upsertError } = await supabase_service_1.supabaseAdmin
            .from('google_connections')
            .upsert({
            user_id: userId,
...
router.post('/sheets/connect', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { url } = req.body;
        if (!url) {
        ...
        const spreadsheetId = (0, google_service_1.extractSpreadsheetId)(url);
        if (!spreadsheetId) {
            res.status(400).json({
        ...
        const { sheets } = await (0, google_service_1.getAuthenticatedSheetsClient)(userId);
        let spreadsheetMeta;
        try {
...
router.get('/sheets/:spreadsheetId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
...
router.post('/sheets/:spreadsheetId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const spreadsheetId = String(req.params.spreadsheetId);
